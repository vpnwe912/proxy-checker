...
func CheckProxy(ctx context.Context, p Proxy, testURL string, timeout time.Duration, allowInsecure bool, useCurl bool, geoEnabled bool, geoProvider string, geoURL string) CheckResult {
	var result CheckResult
	if useCurl {
		if curlResult, ok := CheckProxyWithCurl(ctx, p, testURL, timeout, allowInsecure); ok {
			result = curlResult
		} else {
			result = CheckProxyWithGo(ctx, p, testURL, timeout, allowInsecure)
		}
	} else {
		result = CheckProxyWithGo(ctx, p, testURL, timeout, allowInsecure)
	}
	if result.OK && geoEnabled {
		geoCtx, cancel := context.WithTimeout(ctx, timeout)
		defer cancel()
		geo, err := LookupProxyGeo(geoCtx, result.Proxy, geoProvider, geoURL, allowInsecure)
		if err != nil {
			result.GeoError = err.Error()
		} else {
			result.Geo = geo
		}
	}
...
func CheckProxyWithGo(ctx context.Context, p Proxy, testURL string, timeout time.Duration, allowInsecure bool) CheckResult {
	...
	start := time.Now()
	result := CheckResult{Proxy: p, CheckedAt: start.Format(time.RFC3339)}
	client, err := newProxyHTTPClient(p, timeout, allowInsecure)
	if err != nil {
		result.Error = err.Error()
		result.Duration = time.Since(start)
		return result
...
func LookupProxyGeo(ctx context.Context, p Proxy, geoProvider string, geoURL string, allowInsecure bool) (GeoInfo, error) {
	providers := geoProvidersSequence(geoProvider, geoURL)
	var combined GeoInfo
	var errs []string
	for _, provider := range providers {
		geo, err := lookupGeoWithProvider(ctx, p, provider, allowInsecure)
		if err != nil {
			errs = append(errs, provider+": "+err.Error())
			continue
		}
		combined = mergeGeoInfo(combined, geo)
		if geoProvider != "auto" || (combined.Country != "" && combined.Region != "" && combined.Timezone != "") {
			return combined, nil
		}
	}
	if combined.IP != "" || combined.Country != "" || combined.Region != "" || combined.Timezone != "" {
		return combined, nil
	}
	if len(errs) > 0 {
		return GeoInfo{}, errors.New(strings.Join(errs, "; "))
	}
	return GeoInfo{}, errors.New("geo lookup returned no data")
}

func lookupGeoWithProvider(ctx context.Context, p Proxy, provider string, allowInsecure bool) (GeoInfo, error) {
	client, err := newProxyHTTPClient(p, defaultNetworkTimeout, allowInsecure)
	if err != nil {
		return GeoInfo{}, err
	}
	geoURL := geoProviderURL(provider)
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, geoURL, nil)
	if err != nil {
		return GeoInfo{}, err
	}
	applyGeoHeaders(req, provider)
	resp, err := client.Do(req)
	if err != nil {
		return GeoInfo{}, err
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return GeoInfo{}, fmt.Errorf("geo API returned HTTP %d", resp.StatusCode)
	}
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return GeoInfo{}, err
	}
	return parseGeoResponse(provider, geoURL, body)
}

func parseGeoResponse(provider string, geoURL string, body []byte) (GeoInfo, error) {
	host := ""
	if parsedURL, err := url.Parse(strings.TrimSpace(geoURL)); err == nil {
		host = strings.ToLower(parsedURL.Hostname())
	}
	trimmed := bytes.TrimSpace(body)
	switch {
	case provider == "2ip" || (strings.Contains(host, "2ip.ua") && len(trimmed) > 0 && trimmed[0] != '{'):
		return parse2IPGeo(trimmed)
	case len(trimmed) > 0 && trimmed[0] == '{':
		return parseJSONGeo(trimmed)
	default:
		return parseJSONGeo(trimmed)
	}
}

func applyGeoHeaders(req *http.Request, provider string) {
	switch provider {
	case "2ip":
...
func mergeGeoInfo(base GeoInfo, next GeoInfo) GeoInfo {
	if base.IP == "" {
		base.IP = next.IP
	}
	if base.Country == "" {
...
func geoProvidersSequence(provider string, geoURL string) []string {
	...
	case "ipapi_co":
		return []string{"ipapi_co"}
	case "ip_sb":
		return []string{"ip_sb"}
	case "auto":
		return []string{"2ip_json", "2ip", "ipwhois", "ifconfig", "ipapi_co", "ipapi", "ipinfo", "ip_sb"}
	case "":
		return []string{inferGeoProvider(geoURL)}
	default:
		return []string{"2ip_json", "2ip", "ipwhois", "ifconfig", "ipapi_co", "ipapi", "ipinfo", "ip_sb"}
	}
}

func geoProviderURL(provider string) string {
	switch provider {
	case "2ip":
		return "https://2ip.ua"
	case "2ip_json":
		return "https://2ip.ua/json"
	case "ipapi":
...
func inferGeoProvider(rawURL string) string {
	lower := strings.ToLower(strings.TrimSpace(rawURL))
	switch {
	case strings.Contains(lower, "2ip.ua"):
		if strings.Contains(lower, "/json") {
			return "2ip_json"
		}
		return "2ip"
	case strings.Contains(lower, "ipinfo.io"):
		return "ipinfo"
	case strings.Contains(lower, "ip-api.com"):
		return "ipapi"
	case strings.Contains(lower, "ifconfig.co"):
		return "ifconfig"
	case strings.Contains(lower, "ipwho.is"):
		return "ipwhois"
	case strings.Contains(lower, "ipapi.co"):
		return "ipapi_co"
	case strings.Contains(lower, "api.ip.sb"):
		return "ip_sb"
	default:
		return "auto"
	}
...
func isSupportedGeoProvider(value string) bool {
	switch strings.ToLower(strings.TrimSpace(value)) {
	case "auto", "ipinfo", "2ip", "2ip_json", "ipapi", "ifconfig", "ipwhois", "ipapi_co", "ip_sb":
		return true
	default:
		return false
	}
...
func parseJSONGeo(body []byte) (GeoInfo, error) {
	var payload map[string]any
	if err := json.Unmarshal(body, &payload); err != nil {
		return GeoInfo{}, err
	}
	if status := strings.ToLower(lookupStringAny(payload, "status")); status != "" && status != "success" {
		message := lookupStringAny(payload, "message")
		if message == "" {
			message = status
		}
		return GeoInfo{}, errors.New(message)
	}
	if success := strings.ToLower(lookupStringAny(payload, "success")); success == "false" {
		message := lookupStringAny(payload, "message")
		if message == "" {
			message = "provider returned success=false"
		}
		return GeoInfo{}, errors.New(message)
	}
	geo := GeoInfo{
		IP: strings.TrimSpace(firstNonEmpty(
			lookupStringAny(payload, "ip"),
			lookupStringAny(payload, "query"),
		)),
		Country: strings.TrimSpace(firstNonEmpty(
			lookupStringAny(payload, "country_code"),
			lookupStringAny(payload, "countryCode"),
			lookupStringAny(payload, "country_iso"),
			lookupStringAny(payload, "country_iso_code"),
			lookupStringAny(payload, "country"),
		)),
		Region: strings.TrimSpace(firstNonEmpty(
			lookupStringAny(payload, "regionName"),
			lookupStringAny(payload, "region_name"),
			lookupStringAny(payload, "region"),
			lookupStringAny(payload, "city"),
			lookupStringAny(payload, "region_ua"),
			lookupStringAny(payload, "region_rus"),
			lookupStringAny(payload, "location.city"),
			lookupStringAny(payload, "location.region"),
		)),
		Timezone: strings.TrimSpace(firstNonEmpty(
			lookupStringAny(payload, "timezone.id"),
			lookupStringAny(payload, "timezone"),
			lookupStringAny(payload, "time_zone.name"),
			lookupStringAny(payload, "time_zone"),
		)),
	}
	if len(geo.Country) > 2 {
		if code := strings.TrimSpace(firstNonEmpty(
			lookupStringAny(payload, "country_code"),
			lookupStringAny(payload, "countryCode"),
			lookupStringAny(payload, "country_iso"),
			lookupStringAny(payload, "country"),
			lookupStringAny(payload, "location.country_code"),
		)); len(code) <= 3 {
			geo.Country = code
		}
	}
	if geo.Country == "" && geo.Region == "" && geo.Timezone == "" && geo.IP == "" {
		return GeoInfo{}, errors.New("json geo response does not contain expected fields")
	}
	return geo, nil
}

func parse2IPGeo(body []byte) (GeoInfo, error) {
	...
	if location == "" && fields["ip"] == "" {
		return GeoInfo{}, errors.New("2ip response does not contain expected fields")
	}
	geo := GeoInfo{
	...
	if open := strings.Index(location, "("); open >= 0 {
		if close := strings.Index(location[open:], ")"); close > 0 {
			geo.Country = strings.TrimSpace(location[open+1 : open+close])
		}
	}
	if comma := strings.LastIndex(location, ","); comma >= 0 {
		geo.Region = strings.TrimSpace(location[comma+1:])
	}
	if geo.Region == "" {
		geo.Region = strings.TrimSpace(location)
	}
	return geo, nil
}

func lookupStringAny(value any, path string) string {
...
func newProxyHTTPClient(p Proxy, timeout time.Duration, allowInsecure bool) (*http.Client, error) {
	proxyURL, err := p.URL()
	if err != nil {
		return nil, err
	}
